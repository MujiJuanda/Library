              ...
              <div class="table-container">
                <table class="table table-bordered" id="tabelBuku">
                  ...
                  <tbody <tr>
                      ...
                      <td>
                        <button type="button" class="btn btn-danger btn-sm hapus-buku">Hapus</button>
                      </td>
                    </tr>
                  </tbody>
                </table>
  ...
  <script>
    $(document).ready(function () {
      ...
      $(document).on('click', '.btnTambahInput', function () {
        const newRow = `
            <tr>
                ...
                <td>
                    <input type="text" class="form-control id-buku" placeholder="ID Buku Otomatis" readonly>
                </td>
                <td>
                    <input type="number" class="form-control jumlah-buku" value="1" min="1" required>
                </td>
        ...
        $('#tabelBuku tbody').append(newRow); // Tambahkan baris baru ke tabel
    });
      ...
      $(document).on('input', '.nama-buku', function () {
        ...
        $.ajax({
            url: 'fetch_data/fetch_buku.php', // URL ke PHP
            method: 'GET',
            data: { q: query }, // Kirim query ke server
            dataType: 'json', // Pastikan JSON diterima
            success: function (response) {
                let suggestions = '';

                // Loop respons JSON untuk membuat suggestion
                if (response && response.length > 0) {
                    response.forEach(item => {
                        suggestions += `<div class="suggestion" data-id="${item.id_buku}" data-name="${item.nama_buku}">${item.nama_buku}</div>`;
                    });
                } else {
                    suggestions = '<div class="suggestion">Buku tidak ditemukan</div>';
                }

                // Tampilkan suggestion
                const suggestionBox = $('<div class="suggestion-box"></div>').html(suggestions);
                input.siblings('.suggestion-box').remove(); // Hapus suggestion lama
                input.after(suggestionBox); // Tambahkan suggestion baru
            },
            error: function (xhr, status, error) {
                console.error('Error:', error);
            }
    ...
    $(document).on('click', '.suggestion', function () {
        ...
        input.val(selected.data('name')); // Isi nama buku
        idInput.val(selected.data('id')); // Isi ID buku
        $('.suggestion-box').remove(); // Hapus suggestion setelah memilih
    });

    // Tambahkan logika untuk tombol hapus
    $(document).on('click', '.hapus-buku', function () {
      const rows = $('#tabelBuku tbody tr').length;
  
      // Jika hanya ada satu baris, ubah tombol jadi tambah data buku
      if (rows === 1) {
